...
def unified_dataset_interface(dataset_name, dir, mode='train', context_frame_num=0, border_mode='hard', file_format=None, all_bboxes=None, patch_size=32, color=cv2.IMREAD_COLOR):
...
class TrainForegroundDataset3(Dataset):
    def __init__(self, fore_dir, h_idx, w_idx, transform_raw=transform1, transform_of=transform2):
    ...
    def __getitem__(self, indice):
        ...
        if self.transform_raw is not None:
            return self.transform_raw(raw_fore), self.transform_of(of_fore)
        else:
            return raw_fore, of_fore


class FrameForegroundDataset2(Dataset):
...
class TestForegroundDataset3(Dataset):
    def __init__(self, raw_fore, of_fore, transform_raw=transform1, transform_of=transform2):
    ...
    def __getitem__(self, indice):
        ...
        if self.transform_raw is not None:
            return self.transform_raw(cur_raw_fore), self.transform_of(cur_of_fore)
        else:
...
class ped_dataset(Dataset):
    ...
    def __init__(self, dir, mode='train', context_frame_num=0, border_mode='elastic', file_format='.tif', all_bboxes=None, patch_size=32, color=cv2.IMREAD_COLOR):
        '''
        :param dir: The directory to load UCSD ped1/ped2 dataset
